...
void ordenarPorDistancia(Entidade *jogador) {
    for (int i = 1; i < totalEntidades - 1; i++) {
        for (int j = i + 1; j < totalEntidades; j++) {
            ...
            float dx1 = jogador->pos.x - vetorEntidades[i]->pos.x;
            float dy1 = jogador->pos.y - vetorEntidades[i]->pos.y;
            float dist1 = dx1 * dx1 + dy1 * dy1;

            float dx2 = jogador->pos.x - vetorEntidades[j]->pos.x;
            float dy2 = jogador->pos.y - vetorEntidades[j]->pos.y;
